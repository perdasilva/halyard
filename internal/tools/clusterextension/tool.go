...
func (t *listClusterExtensionsTool) Name() string {
	return "list_cluster_extensions"
}
...
func (t *listClusterExtensionsTool) Call(ctx context.Context, _ string) (string, error) {
	resp, err := t.client.Resource(clusterExtensionGvr).List(ctx, v1.ListOptions{})
	if err != nil {
...
type getClusterExtensionsTool struct {
	client *dynamic.DynamicClient
}

func (t *getClusterExtensionsTool) Name() string {
	return "get_cluster_extension"
}

func (t *getClusterExtensionsTool) Description() string {
	return "Get a specific ClusterExtension. Input must be in the format: { \"name\": <clusterextension-name> }. For example: { \"name\": \"argocd\" }."
}

func (t *getClusterExtensionsTool) Call(ctx context.Context, arguments string) (string, error) {
	var args struct {
	...
	resp, err := t.client.Resource(clusterExtensionGvr).Get(ctx, args.Name, v1.GetOptions{})
	if err != nil {
...
func NewGetTool(dynClient *dynamic.DynamicClient) llmTools.Tool {
	return &getClusterExtensionsTool{
		client: dynClient,
	}
}

type generateClusterExtensionsTool struct {
	llm llms.Model
}

func (t *generateClusterExtensionsTool) Name() string {
	return "generate_cluster_extension"
}

func (t *generateClusterExtensionsTool) Description() string {
	return "Generate a ClusterExtension resource given an intent. Input should be in the form of a string. For instance:" +
		"generate a cluster extension that installs argocd-operator in namespace argocd using the argocd-installer service account"
}

const generatePrompt = `You are a helpful AI assistant and an expert in generating correct ClusterExtension CRs from the following definition:
...
Given the user prompt, generate a valid ClusterExtension CR in yaml format that answers the prompt. Return only the yaml
definition, no preamble or explanation.`

func (t *generateClusterExtensionsTool) Call(ctx context.Context, arguments string) (string, error) {
	content := []llms.MessageContent{
		llms.TextParts(llms.ChatMessageTypeSystem, generatePrompt),
		llms.TextParts(llms.ChatMessageTypeHuman, arguments),
	}
	answer, err := t.llm.GenerateContent(ctx, content)
	if err != nil {
		return fmt.Sprintf("error generating ClusterExtension CR: %v", err), nil
	}
	return answer.Choices[0].Content, nil
}

func NewGenerateTool(llm llms.Model) llmTools.Tool {
	return &generateClusterExtensionsTool{
		llm: llm,
	}
}
